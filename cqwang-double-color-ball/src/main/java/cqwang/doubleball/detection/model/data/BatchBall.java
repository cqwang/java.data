...
public class BatchBall {
    private Map<Integer, BatchBall> cache = new HashMap<>();
     ...
     * @return
     */
    public SingleResult getMaxDataFrequency(BatchBall batchBall, BatchPredictOption option) {
        for (int index = sortedList.size() - 1; index >= 0; index--) {
            var item = sortedList.get(index);
            if (option.isBlock(item.getData())) {
                continue;
            }

            if(option.isAllow(item.getData())) {
    ...
    public SingleResult getMinDataFrequency(BatchBall batchBall, BatchPredictOption option) {
     ...
     * @return
     */
    public BatchBall sub(int sampleSize) {
